...
-- Utility Functions
local Util = { }; do
    local ArrowCursor = "rbxasset://textures/Cursors/KeyboardMouse/ArrowCursor.png";

    function Util.ForceArrowCursor()
        local Player = Players.LocalPlayer;
        if (not Player) then return; end

        PCall(function()
            local Mouse = Player:GetMouse();
            if (Mouse) then Mouse.Icon = ArrowCursor; end
        end)
    end
    ...
    function Util.Button(Parent, Name, Position, Size, Text, ZIndex)
        ...
        Object.MouseEnter:Connect(function()
            Util.ForceArrowCursor();
            TaskDefer(Util.ForceArrowCursor);
        end)
    ...
    function Util.SideGradient(Parent, LeftColor, RightColor)
        local Object = InstanceNew("UIGradient");
        Object.Color    = ColorSequence.new(LeftColor, RightColor);
        Object.Rotation = 0;
        Object.Parent   = Parent;
        return Object;
    end
