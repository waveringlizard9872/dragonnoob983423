...
local function ResolveLoaderImage()
    ...
    local Success, Result = PCall(function()
        ...
        if (not isfile(ImagePath)) or (#readfile(ImagePath) < 1000) then
            writefile(ImagePath, game:HttpGet(`{Repository}/assets/noFilter.jpg`));
        end
...
function Library.loader(Options)
    ...
    local ChangeInput = Util.Create("TextBox", {
        Name                   = "TextBox",
        Parent                 = ChangeBox,
    ...
    Util.ApplyFont(ChangeInput, Layout.TextSize, false);
