return function(Environment)
    ...
    local StringGSub       = Environment.StringGSub;
...
function Library:Notify(Text, Options)
    local WindowObject = self.Windows[#self.Windows];
    if (not WindowObject) or (not WindowObject.Notify) then
        return nil;
    end

    return WindowObject:Notify(Text, Options);
end
...
function Library.ConfigManager:Save(Config, ForceCreate)
    if (not Config) or (ToString(Config) == "") then return false; end
    if (not ConfigUtil.EnsureDirectory())         then return false; end

    if (not ForceCreate) and (self.CurrentlyLoadedConfig) and (self.CurrentlyLoadedConfig.Name ~= Config) and (not self:IsDirty()) then
...
function Library.ConfigManager:Create(Config)
    return self:Save(Config, true);
end
...
function Library.ConfigManager:SetAutoload(Config)
    if (not Config) or (ToString(Config) == "") or (not ConfigUtil.EnsureDirectory()) then return false; end

    local Found = self:Find(Config);
    if (not Found) then
        if (not self:Create(Config)) then return false; end
        Found = self:Find(Config);
    end
...
function Library.ConfigManager:LoadAutoload()
    ...
    local Config = StringGSub(readfile(self.Autoload), "^%s*(.-)%s*$", "%1");
